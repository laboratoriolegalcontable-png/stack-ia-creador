...
description: >
  ...
  100% no-destructivo: respalda antes de escribir, nunca pisa ni rompe nada.
version: 1.1.0
---

# Kairos Forge — Fabrica de Skills y Subagentes

## Garantia no-destructiva (regla de Diego)

Forge NUNCA pisa ni rompe nada. Antes de escribir cualquier archivo respalda el
existente en `~/.claude/.forge-backups/<timestamp>/`. La memoria solo se siembra si
falta. El registry es append-only. Detalle completo en `INTEGRATION.md`.
...
```
Guarda un aprendizaje en memoria persistente (CLAUDE.md + ~/.claude/memory/ + backend opcional).
...
Forge se integra con Kairos Legendario (modo boss):

- En modo boss: forge ejecuta sin pedir confirmacion (la garantia no-destructiva NO se desactiva)
- Forge puede registrar skills creadas en la memoria persistente para acceso cross-sesion
- Detalle del puente en `../kairos-legendario/FORGE-BRIDGE.md`
- Los endpoints/IDs concretos de notificacion viven en el CLAUDE.md privado del proyecto
