...
install_skill() {
  ...
  # REGLA CRITICA: nunca pisar un skill existente. Solo se sobreescribe si
  # FORGE_FORCE=1. Asi una instalacion repetida es idempotente y no destructiva.
  if [ -f "$target" ] && [ "${FORGE_FORCE:-0}" != "1" ]; then
    log "Ya existe, no se pisa: $skill_name (usar FORGE_FORCE=1 para forzar)"
    return 0
  fi

  if [ -n "$REPO_ROOT" ] && [ -f "$REPO_ROOT/.claude/skills/$skill_name/SKILL.md" ]; then
    # Modo repo local — copiar archivos
    cp "$REPO_ROOT/.claude/skills/$skill_name/SKILL.md" "$target"
...
echo ""
for skill in kairos-forge kairos-sentinel kairos-genesis kairos-memory-v4; do
  install_skill "$skill"
done

# Copiar REFERENCE.md de forge si existe en repo local (sin pisar)
if [ -n "$REPO_ROOT" ] && [ -f "$REPO_ROOT/.claude/skills/kairos-forge/REFERENCE.md" ]; then
  if [ ! -f "$SKILLS_DIR/kairos-forge/REFERENCE.md" ] || [ "${FORGE_FORCE:-0}" = "1" ]; then
