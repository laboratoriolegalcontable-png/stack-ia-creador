...
export default function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  ...
  // No external API keys in local mode
  const apiKeys: any[] = [];
  const generateKey = async (_args: any) => { toast.info('API keys not needed in local mode'); return null; };
  const revokeKey = async (_args: any) => {};

  // LLM keys derived from server config (env vars)
  const userLLMKeys: any[] = [];
  const upsertLLMKey = async (_args: any) => { toast.info('Set LLM keys in .env.local'); };
  const deleteLLMKey = async (_args: any) => {};
  const toggleLLMKeyActive = async (_args: any) => {};
  ...
  const addMCPServer = async (args: any) => { toast.info('MCP servers are configured in mcp-registry.ts'); };
  const updateMCPServer = async (_args: any) => {};
  const deleteMCPServer = async (_args: any) => { toast.info('MCP servers are configured in mcp-registry.ts'); };
  const toggleMCPEnabled = async (_args: any) => {};
  const seedOfficialMCPs = async (_args: any) => {};
  const updateConnectionStatus = async (_args: any) => {};
  const cleanupOfficialMCPs = async (_args: any) => {};
                  ...
                  <div className="flex gap-8 mb-12">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (result) {
                          setGeneratedKey((result as any).key);
                          setNewKeyName("");
                          if (typeof window !== 'undefined') {
                            sessionStorage.setItem('latest_api_key', (result as any).key);
                          }
                        }
            ...
            <div className="grid grid-cols-2 gap-12">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  {server.accessToken && <span className="text-xs text-black-alpha-32">&bull;&bull;&bull;&bull;&bull;&bull;&bull;</span>}
                </div>
              </div>
            </div>
