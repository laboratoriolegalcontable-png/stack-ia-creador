...
export function renderHeatmap() {
  ...
  // recordActivity() is NOT called here — opening the panel is not a session event.
  // Activity is recorded once at startup via init.js.
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
      ...
      <button id="hm-close" style="background:none;border:none;color:#6b7280;cursor:pointer;font-size:1.2rem">&times;</button>
    </div>
    <p style="font-size:0.78rem;color:#9ca3af;margin-bottom:1rem">Actividad por hora (hora local) en los últimos 7 días.</p>
    <div style="overflow-x:auto">
      <table style="border-collapse:collapse;font-size:0.68rem;width:100%">
        <thead><tr>
          ...
          ${Array.from({length:24}, (_, i) => `<th style="color:${[0,6,12,18,23].includes(i)?'#9ca3af':'transparent'};padding:0.1rem;text-align:center">${i}h</th>`).join('')}
        </tr></thead>
        <tbody>
          ${recentDates.map(date => `<tr>
            ...
            ${Array.from({length:24}, (_, h) => {
              ...
              const bg = count > 0 ? `rgba(99,102,241,${(0.15+intensity*0.85).toFixed(2)})` : '#1f2937';
  ...
  panel.querySelector('#hm-close').addEventListener('click', () => panel.remove());
