...
/**
 * @param {string} period  — e.g. 'Sprint 12' or 'Semana 2025-W21'
 * @param {string} wentWell
 * @param {string} improvements
 * @param {string} actionItems  — newline-separated list
 * @returns {object}
 */
...
/** Returns retros sorted by date descending (newest first). */
...
/** @returns {{ total: number, openActions: number }} */
export function getRetroStats() {
  const list = loadRetros();
  return {
    total: list.length,
    openActions: list.filter(r => !r.actionsDone).length,
  };
}
