...
/**
 * Log energy level for the current hour.
 * @param {number} level — integer 1-5
 * @returns {{ date: string, hour: number, level: number }}
 */
...
/**
 * Returns the energy log for the last `days` days.
 * @param {number} days
 * @returns {Array<{ date: string, hour: number, level: number }>}
 */
export function getEnergyHistory(days = 7) {
  ...
  return Object.entries(data)
    ...
    .map(([k, level]) => {
      ...
      return { date, hour: Number(hourStr), level: Number(level) };
    })
    .sort((a, b) => a.date.localeCompare(b.date) || a.hour - b.hour);
}

/**
 * Returns the hour (0-23) with the highest average energy across all history.
 * @returns {number|null}
 */
...
/**
 * @returns {{ avg: number, peak: number, trend: 'up'|'down'|'stable' }}
 */
...
export function renderEnergyPanel() {
  ...
  for (let h = 0; h < 24; h++) {
    ...
    if (byHour[h] !== undefined) {
      const barH = Math.round((byHour[h] / 5) * maxH);
      const bar = document.createElement('div');
      const levelColors = ['','#374151','#4b5563','#059669','#d97706','#7c3aed'];
      bar.style.cssText = `width:100%;height:${barH}px;background:${levelColors[byHour[h]]};border-radius:2px 2px 0 0;transition:height 0.2s`;
