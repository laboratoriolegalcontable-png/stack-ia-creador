...
export function addTerm(word, definition, example = '', tags = []) {
  ...
  const term = {
    id,
    word: String(word).slice(0, 200),
    definition: String(definition).slice(0, 2000),
...
export function markLearned(id) {
  const data = loadVocab();
  if (data[id]) { data[id].status = 'learned'; saveVocab(data); }
}

export function resetTerm(id) {
  const data = loadVocab();
  if (data[id]) { data[id].status = 'learning'; saveVocab(data); }
}
...
/**
 * @param {'all'|'learned'|'learning'} filter
 * @returns {object[]}
 */
...
export function deleteTermEntry(id) {
  const data = loadVocab();
  delete data[id];
  saveVocab(data);
}

/** @returns {{ total: number, learned: number, learning: number }} */
...
export function renderVocabPanel() {
  ...
  inWord.placeholder = 'Término / palabra...';
  ...
  const inDef = document.createElement('textarea');
  inDef.placeholder = 'Definición...';
  ...
  inEx.placeholder = 'Ejemplo de uso (opcional)...';
  ...
  inTags.placeholder = 'Tags separados por coma (ej: legal, tech)...';
  ...
  addBtn.textContent = '+ Agregar término';
  ...
  function renderList() {
    ...
    if (terms.length === 0) {
      ...
      empty.textContent = 'Sin términos.';
    ...
    terms.forEach(t => {
      ...
      top.style.cssText = 'display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.4rem';

      const wordEl = document.createElement('div');
      wordEl.style.cssText = 'color:#e5e7eb;font-size:0.9rem;font-weight:600';
      ...
      defEl.style.cssText = 'font-size:0.82rem;color:#d1d5db;margin-bottom:0.3rem';
      ...
      if (t.example) {
        ...
        exEl.style.cssText = 'font-size:0.78rem;color:#6b7280;font-style:italic;margin-bottom:0.35rem';
        exEl.textContent = `Ej: ${t.example}`;
      ...
      if (t.tags && t.tags.length > 0) {
        ...
        tagsRow.style.cssText = 'display:flex;flex-wrap:wrap;gap:0.3rem;margin-bottom:0.4rem';
        t.tags.forEach(tag => {
          const chip = document.createElement('span');
          chip.style.cssText = 'font-size:0.68rem;padding:0.1rem 0.4rem;background:#1e3a5f;color:#60a5fa;border-radius:4px';
      ...
      if (t.status === 'learning') {
        ...
        learnBtn.style.cssText = 'padding:0.25rem 0.6rem;background:#065f46;color:#34d399;border:none;border-radius:5px;cursor:pointer;font-size:0.75rem';
        learnBtn.addEventListener('click', () => { markLearned(t.id); renderList(); refreshStats(); });
        btns.appendChild(learnBtn);
      } else {
        const resetBtn = document.createElement('button');
        resetBtn.textContent = '↩ Reset';
        resetBtn.style.cssText = 'padding:0.25rem 0.6rem;background:#374151;color:#9ca3af;border:none;border-radius:5px;cursor:pointer;font-size:0.75rem';
        resetBtn.addEventListener('click', () => { resetTerm(t.id); renderList(); refreshStats(); });
        btns.appendChild(resetBtn);
      }

      const delBtn = document.createElement('button');
      delBtn.textContent = '✕';
      delBtn.style.cssText = 'padding:0.25rem 0.45rem;background:#450a0a;color:#f87171;border:none;border-radius:5px;cursor:pointer;font-size:0.75rem';
      delBtn.addEventListener('click', () => { deleteTermEntry(t.id); renderList(); refreshStats(); });
