...
/**
 * Days between two YYYY-MM-DD strings using local midnight dates.
 * new Date(y,m,d) = local midnight (no UTC parse bug).
 * Math.round handles DST days (23h or 25h).
 */
...
/**
 * Yesterday as YYYY-MM-DD via setDate()-1 (correct across DST, unlike -86400000ms).
 */
...
export function checkIn(id) {
  ...
  if (h.frequency === 'weekly') {
    const daysSinceLast = h.lastCheckedIn ? daysBetweenLocalDates(h.lastCheckedIn, today) : Infinity;
